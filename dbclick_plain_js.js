...
function setupDoubleClick(websiteUrl, dictionary, showFirstEntry, areaClass, maxAllowedWords, target, urlCallback) {
    ...
    var showLayer = function(e) {
        ...
        // ADDED
        var definition_layer = document.querySelector("#definition_layer");
        if (lookup != null && lookup.replace("/\s/g", "").length > 0) {
            ...
            if (!definition_layer) {
                ...
                // REPLACED
                definition_layer = document.createElement("div");
                definition_layer.id = "definition_layer";
                definition_layer.setAttribute("style", 'position:absolute; cursor:pointer;');
                definition_layer.innerHTML += "<img src='" + imageUrl + "' alt='' title=''/>";

                document.body.appendChild(definition_layer);
            }
            ...
            // REPLACED
            definition_layer.style.left = `${e.pageX - 30}px`;
            definition_layer.style.top = `${e.pageY - 40}px`;
    ...
    // REPLACED
    document.querySelector(area).onmouseup = showLayer;
}
